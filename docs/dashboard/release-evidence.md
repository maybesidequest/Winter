...
| P3-CONTROL | Durable operations, selectors, previews, and canonical Bot authorization | InterChat `2720242c`, `8cc84ba8`, `960e4076`, `9cf99ba9`, `cffc514d`, `9f7950e`, `239a558d`, `ccf8f953`, `f77516ce`, `dee5b6e6`, `fab45595`, `38717236`, `2c884935`, `57139110`, `44e8dbdf`; selector suite — 7 passed; operation service suite — 5 passed; connection projection suite — 2 passed; targeted Bot Ruff/Pyright passed; Bot Hub permission checks, outbox management reads, connection command mutations, Hub info/rules/disconnect lookups, deleted-channel cleanup, and connection uniqueness reads now route through Control Plane; canonical connection reads expose the latest linked operation; bridge views now display desired connection state separately from observed health | InterChat local worktree | implementation owner | 2026-09-01 | passed locally; repository-wide Bot Pyright crashes pre-existing; Bot locale submodule unavailable for affected pytest collection |
...
| P3-UX-STATES | Truthful bridge state and health | Winter `b12d0eb`, `4b30fb1`, `5946573`, `7959236`; server bridge resources preserve Control Plane observed health and operation metadata, UI separates enabled/paused desired state from health attention, bridge search no longer enumerates raw channel IDs, unavailable names/timestamps remain explicit instead of fabricated, disconnect uses an accessible typed confirmation, and Hub bridge rows show observed health separately. The current checkpoint adds typed operation polling from the canonical operation ID: pending/running, partial, failed, cancelled, completed, and unavailable are distinct states; only pending/running poll. `react-router typegen`, `tsc`, and `operationPresentation.test.ts` (3 passed) passed | Winter local worktree | implementation owner | 2026-09-01 | passed locally |
