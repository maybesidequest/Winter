...
| P3-OPERATIONS | Durable partial outcomes and service-only progress | InterChat `9cf99ba9`, `cffc514d`; operation progress accepts terminal `PARTIAL`, emits a durable transition, rejects human actors, and returns typed `NOT_FOUND`; `uv run pytest apps/control_plane/tests/test_operation_service.py apps/control_plane/tests/test_control_operations.py -q` — 11 passed | InterChat local worktree | implementation owner | 2026-09-01 | passed locally |
