...
| P3-PROTO | Additive protobuf contracts | protobuf `558bf43`, `6533825`; `protoc --include_imports` and `git diff --check` passed. `ResolveConnectionTarget` was added compatibly to `ConnectionService`; InterChat Python and Winter static TypeScript bindings were regenerated | protobuf local worktree | implementation owner | 2026-09-01 | partial — Buf unavailable |
