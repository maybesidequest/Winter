...
| P3-CONTROL | Durable operations, selectors, previews, and canonical Bot authorization | InterChat `2720242c`, `8cc84ba8`, `960e4076`, `9cf99ba9`, `cffc514d`, `9f7950e`, `239a558d`, `ccf8f953`, `f77516ce`, `dee5b6e6`, `fab45595`, `38717236`; selector suite — 7 passed; operation service suite — 5 passed; targeted Bot Ruff/Pyright passed; Bot Hub permission checks, outbox management reads, connection command mutations, and hub info/rules/disconnect lookups now route through Control Plane; bridge views now display desired connection state separately from observed health | InterChat local worktree | implementation owner | 2026-09-01 | passed locally; repository-wide Bot Pyright crashes pre-existing; Bot locale submodule unavailable for affected pytest collection |
...
| P3-UX-STATES | Truthful bridge state and health | Winter `b12d0eb`, `4b30fb1`; server bridge resources preserve Control Plane observed health and operation metadata, UI separates enabled/paused desired state from health attention, bridge search no longer enumerates raw channel IDs, and unavailable names/timestamps remain explicit instead of fabricated; generated typecheck and structural validation passed | Winter local worktree | implementation owner | 2026-09-01 | passed locally |
