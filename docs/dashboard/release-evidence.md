...
| P3-CONTROL | Durable operations, selectors, previews, and canonical Bot authorization | InterChat `2720242c`, `8cc84ba8`, `960e4076`, `9cf99ba9`, `cffc514d`, `9f7950e`, `239a558d`, `ccf8f953`; selector suite — 7 passed; operation service suite — 5 passed; targeted Bot Ruff/Pyright passed; Bot Hub permission checks, outbox management reads, and connection command mutations now route through Control Plane | InterChat local worktree | implementation owner | 2026-09-01 | passed locally; repository-wide Bot Pyright crashes pre-existing; Bot locale submodule unavailable for affected pytest collection |
...
| P3-OPERATIONS | Durable partial outcomes and service-only progress | InterChat `9cf99ba9`, `cffc514d`, `9e3166bb`, `ccf8f953`; operation progress accepts terminal `PARTIAL`, emits a durable transition, rejects human actors, returns typed `NOT_FOUND`, and disconnect cleanup is correlated to a durable operation with started/completed/failed outbox events; Control Plane operation suites — 11 passed; disconnect correlation test — 1 passed; Bot typed client correlation test — 1 passed | InterChat local worktree | implementation owner | 2026-09-01 | passed locally |
